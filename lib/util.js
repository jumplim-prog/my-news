...
export function interestMaxAgeHours(interest, site) {
  return interest.maxAgeHours ?? site?.maxAgeHours ?? 72;
}

export function isWithinMaxAge(article, maxAgeHours, now = Date.now()) {
  if (!article.publishedAt) return true;
  const published = article.publishedAt.getTime
    ? article.publishedAt.getTime()
    : new Date(article.publishedAt).getTime();
  if (Number.isNaN(published)) return true;
  return published >= now - maxAgeHours * 3600 * 1000;
}
